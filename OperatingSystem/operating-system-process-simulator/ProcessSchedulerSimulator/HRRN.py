class HRRN:
    ...
    def run(self):
        ...
        while readyQ or processList or self.check(coreList): #readyQ or process가 빌때까지 or 모든 코어가 일하지 않을 때까지
            ...
            for p in readyQ:
                p.ratio = ((time-p.arrival_time)+p.work_load)/p.work_load #Aging(일하려는 시간보다 기다린 시간의 비율)

            #ratio(Aging)비율이 높으면 먼저 처리 but 같으면 일하는 시간이 적은 순 먼저 빼주기 
            readyQ.sort(key = lambda p : (-p.ratio, p.work_load))
            ...
            newReadyQ = [] #현재 일하지 않고 있는 코어를 담을 리스트
